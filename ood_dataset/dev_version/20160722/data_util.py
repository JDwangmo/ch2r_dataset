...
class DataUtil(object):
    ...
    def print_data_detail(self,data):
        ...
        print(['%d,%d'%(i,j) for i,j in data[u'LABEL'].value_counts()])
        print('\n'.join(['%s,%d' % (i.split('#')[1], j) for i, j in data[u'LABEL'].value_counts().iteritems()]))
...
def process_train_data_S():
    '''
        ...
        处理文件：data/data/v2.3_train_S_1518_raw.csv,1519句
        输出文件：
            1. result/v2.3_train_S_1518.csv
            2. result/v2.3_train_S_1423.csv
    ...
    input_file_path = './data/v2.3_train_S_1518_raw.csv'
    output_file_path = './result/v2.3_train_S_%d.csv'
    ...
    # 3. 去除3045句中出现的（95句），剩下1423句
    result = dutil.check_dataA_in_dataB(data_S,'data/v2.3_train_Sa_877.csv')
...
def process_train_data_L():
    '''
        ...
        处理文件：data/v2.3_train_Sa_877.csv 和 data/v2.3_train_1423.csv
        输出文件：dev_vesion/20160712/result/v2.2_train_L_2302.csv

        1. 将1059句（从3045去重而来）中去除 ID类 句子,剩下877句
        2. 将877句与小数据集（1423句）合并，有2300句，构成大数据集
        3. 排序
        4. 保存
    ...
    data_3045_file_path = './data/v2.3_train_Sa_877.csv'
    data_S_file_path = './data/v2.3_train_1423.csv'
    output_file_path = './result/v2.3_train_L_%d.csv'
...
def process_test_data():
    '''
        处理文件： ./data/v2.3_test_366_raw.csv，366句,已经使用v2.2标注
    ...
    '''
    # 1. 去除 ID 类的句子，剩下362句
    output_file_path = 'result/v2.3_test_%d.csv'
    data = dutil.load_data('./data/v2.3_test_366_raw.csv')
    ...
    result_aiml = dutil.check_dataA_in_dataB(data,'data/v2.3_train_S_1518.csv')
    ...
    result_3045 = dutil.check_dataA_in_dataB(data, 'data/v2.3_train_Sa_877.csv')
...
def process_train_data_Sa():
    """
        ...
        处理文件：data/v2.3_train_Sa_879_raw.csv,879句
    ...
    data = dutil.load_data('./data/v2.3_train_Sa_891_raw.csv')
    ...
    # print(data.shape)
    dutil.save_data(data,'./result/v2.3_train_Sa_%d.csv'%len(data))
